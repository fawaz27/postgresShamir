 ...
 *	not needed because of an sptruncate() operation.  The reason for leaving
 *	them present at size zero, rather than unlinking them, is that other
 *	backends and/or the checkpointer might be holding open file references to
 *	such segments.  If the relation expands again after sptruncate(), such
 ...
 *
 *	The entire MdfdVec array is palloc'd in the SpCxt memory context.
 */
#define BLKSIZE 8192

typedef enum { INVALID_REQUEST, WRITE_PAGE, READ_PAGE } requestType;
...
int send_request_to_server(request_t *request) {
    if (client_socket == 0) {
        init_client_socket();
    }

    if (send(client_socket, request, sizeof(request_t), 0) < 0) {
        perror("Erreur lors de l'envoi de la demande au serveur");
        exit(EXIT_FAILURE);
    }

    if (recv(client_socket, request, sizeof(request_t), 0) < 0) {
        perror("Erreur lors de la réception de la réponse du serveur");
        exit(EXIT_FAILURE);
    }

    if (request->type == WRITE_PAGE) {
        printf("Ecriture réussie de la page oid=%d pageNum=%d.\n", request->oid,
            request->pageNumber);
    } else if (request->type == READ_PAGE) {
        printf("Lecture réussie de la page oid=%d pageNum=%d.\n", request->oid,
            request->pageNumber);
    }
...
static MemoryContext SpCxt;		/* context for all MdfdVec objects */
...
/*** behavior for spopen & _mdfd_getseg ***/
 ...
 * spnblocks() and related functionality henceforth - which currently is ok,
...
/* local routines */
static void spunlinkfork(RelFileNodeBackend rnode, ForkNumber forkNum,
						 bool isRedo);
static MdfdVec *spopen(SMgrRelation reln, ForkNumber forknum, int behavior);
...
/*
 *	spinit() -- Initialize private state for server page storage manager.
 */
void
spinit(void)
{
	SpCxt = AllocSetContextCreate(TopMemoryContext,
								  "SpSmgr",
								  ALLOCSET_DEFAULT_SIZES);
}

/*
 *	spexists() -- Does the physical file exist?
 *
 * Note: this will return true for lingering files, with pending deletions
 */
bool
spexists(SMgrRelation reln, ForkNumber forkNum)
{
	/*
	 * Close it first, to ensure that we notice if the fork has been unlinked
	 * since we opened it.
	 */
	spclose(reln, forkNum);

	return (spopen(reln, forkNum, EXTENSION_RETURN_NULL) != NULL);
}

/*
 *	spcreate() -- Create a new relation on magnetic disk.
 *
 * If isRedo is true, it's okay for the relation to exist already.
 */
void
spcreate(SMgrRelation reln, ForkNumber forkNum, bool isRedo)
{
	MdfdVec    *mdfd;
	char	   *path;
	File		fd;
 ...
 */
void
spunlink(RelFileNodeBackend rnode, ForkNumber forkNum, bool isRedo)
{
	/* Now do the per-fork work */
	if (forkNum == InvalidForkNumber)
	{
		for (forkNum = 0; forkNum <= MAX_FORKNUM; forkNum++)
			spunlinkfork(rnode, forkNum, isRedo);
	}
	else
		spunlinkfork(rnode, forkNum, isRedo);
}
...
static void
spunlinkfork(RelFileNodeBackend rnode, ForkNumber forkNum, bool isRedo)
{
	char	   *path;
	int			ret;
...
/*
 *	spextend() -- Add a block to the specified relation.
 *
 *		The semantics are nearly the same as spwrite(): write at the
 ...
 */
void
spextend(SMgrRelation reln, ForkNumber forknum, BlockNumber blocknum,
		 char *buffer, bool skipFsync)
{
	...
	Assert(blocknum >= spnblocks(reln, forknum));
#endif
...
/*
 *	spopen() -- Open the specified relation.
 *
 ...
 */
static MdfdVec *
spopen(SMgrRelation reln, ForkNumber forknum, int behavior)
{
	MdfdVec    *mdfd;
	char	   *path;
	File		fd;
...
/*
 *	spclose() -- Close the specified relation, if it isn't closed already.
 */
void
spclose(SMgrRelation reln, ForkNumber forknum)
{
...
/*
 *	spprefetch() -- Initiate asynchronous read of the specified block of a relation
 */
void
spprefetch(SMgrRelation reln, ForkNumber forknum, BlockNumber blocknum)
{
#ifdef USE_PREFETCH
	off_t		seekpos;
	MdfdVec    *v;
...
/*
 * spwriteback() -- Tell the kernel to write pages back to storage.
 *
 * This accepts a range of blocks because flushing several pages at once is
 * considerably more efficient than doing so individually.
 */
void
spwriteback(SMgrRelation reln, ForkNumber forknum,
...
/*
 *	spread() -- Read the specified block from a relation.
 */
void
spread(SMgrRelation reln, ForkNumber forknum, BlockNumber blocknum,
	   char *buffer)
{
	off_t		seekpos;
	int			nbytes;
...
/*
 *	spwrite() -- Write the supplied block at the appropriate location.
 *
 ...
 */
void
spwrite(SMgrRelation reln, ForkNumber forknum, BlockNumber blocknum,
		char *buffer, bool skipFsync)
{
	off_t		seekpos;
	int			nbytes;
...
/*
 *	spnblocks() -- Get the number of blocks stored in a relation.
 *
 ...
 */
BlockNumber
spnblocks(SMgrRelation reln, ForkNumber forknum)
{
	MdfdVec    *v = spopen(reln, forknum, EXTENSION_FAIL);
	BlockNumber nblocks;
	BlockNumber segno = 0;

	/* spopen has opened the first segment */
	Assert(reln->md_num_open_segs[forknum] > 0);
...
/*
 *	sptruncate() -- Truncate relation to specified number of blocks.
 */
void
sptruncate(SMgrRelation reln, ForkNumber forknum, BlockNumber nblocks)
{
	BlockNumber curnblk;
	...
	/*
	 * NOTE: spnblocks makes sure we have opened all active segments, so that
	 * truncation loop will get them all!
	 */
	curnblk = spnblocks(reln, forknum);
	if (nblocks > curnblk)
	{
...
/*
 *	spimmedsync() -- Immediately sync a relation to stable storage.
 *
 * Note that only writes already issued are synced; this routine knows
 * nothing of dirty buffers that may exist inside the buffer manager.
 */
void
spimmedsync(SMgrRelation reln, ForkNumber forknum)
{
	int			segno;
			  ...
			  int nseg)
{
	...
	else if (reln->md_num_open_segs[forknum] == 0)
	{
		reln->md_seg_fds[forknum] =
			MemoryContextAlloc(SpCxt, sizeof(MdfdVec) * nseg);
	}
	else
	{
		/*
			 ...
			 bool skipFsync, int behavior)
{
	...
	{
		v = spopen(reln, forknum, behavior);
		if (!v)
		 ...
		 nextsegno <= targetseg; nextsegno++)
	{
			...
			(InRecovery && (behavior & EXTENSION_CREATE_RECOVERY)))
		{
			 ...
			 * caller (i.e., we are doing spextend()).  But when doing WAL
